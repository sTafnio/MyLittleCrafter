...
public class SessionInfo
{
    ...
    public double TotalSessionCost => TotalCostPerCurrency.Values.Sum();
    ...
    public void ToImGUI()
    {
        ...
        ImGui.Text($"Total Cost: {TotalSessionCost:F2}c");
        ImGui.Separator();

        // Display crafts summary - make sortable
        ...
        if (TotalCurrencyUsed.Count > 0 && ImGui.BeginTable("CurrencyTable", 3, ImGuiTableFlags.Sortable | ImGuiTableFlags.Borders | ImGuiTableFlags.Resizable))
        {
            ...
            if (ImGui.TableGetSortSpecs() is { } sortSpecs)
            {
                ...
                foreach (var currency in sortedCurrency)
                {
                    ...
                    double costPerUnit = TotalCostPerCurrency.GetValueOrDefault(currencyName, 0) / amount;
